...
class Raven {
  constructor() {
    ...
    this.flightSpeed = Math.random() * 1.5 + 1.5;
  } 
  ...
  update(deltaTime) {
    ...
    this.x -= this.directionX * this.flightSpeed;
    this.y += this.directionY;
    
    if (score >= 50) this.flightSpeed = Math.random() * 2 + 2;
...
class Explosion {
  ...
  draw() {
    ctx.drawImage(this.image, this.frame * this.spriteWidth, 0, this.spriteWidth,
      this.spriteHeight, this.x - this.size * 0.1, this.y - this.size * 0.2, this.size, this.size);
  }
...
function animate(timestamp=0) {
  ...
  particles = particles.filter(raven => !raven.markedForDeletion);
